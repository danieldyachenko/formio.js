  ...
  javaScriptValue: function javaScriptValue(title, property, propertyJSON, weight, exampleHTML, exampleJSON) {
    ...
    var components = [this.logicVariablesTable(additionalParams), {
      ...
      components: [{
        ...
        /* eslint-disable prefer-template */
        content: '<p>Выполнение пользовательской логики с использованием <a href="http:jsonlogic.com" target="_blank">JSONLogic<a>.<p>' + '<p>Полная <a href="https:lodash.comdocs" target="_blank">Lodash<a> поддерживается с помощью "_" перед каждой операцией, например <code>{"_sum": {var: "data.a"}}<code><p>' + exampleJSON
        /* eslint-enable prefer-template */
